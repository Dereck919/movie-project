  ...
  try {
    ...
    const movies = await page.evaluate(() => {
      ...
      movieElements.forEach((movie) => {
        //Title
        const titleElement = movie.querySelector(
          "li.shared-movie-showtimes h3.shared-movie-showtimes__movie-title a"
        );
        ...
        let runtime = "N/A";
        if (runtimeEl) {
          const runtimeTextNode = Array.from(runtimeEl.childNodes).find(
            (node) =>
        ...
        //Rating
        const ratingEl = movie.querySelector(
          "data.shared-showtimes__movie-rating"
        );
